    ...
    public DataTable GetAdminStats(int instituteId, int sessionId)
    {
        SqlCommand cmd = new SqlCommand(@"
            SELECT 
                ...
                (SELECT COUNT(*) FROM LevelSemesterSubjects WHERE InstituteId = @InstId AND IsMandatory = 1 AND SessionId = @SessId) as MandatoryCount,
                (SELECT COUNT(*) FROM AssignStudentSubject WHERE InstituteId = @InstId AND SessionId = @SessId) as TotalEnrollments,
