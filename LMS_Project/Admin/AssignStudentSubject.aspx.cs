...
namespace LearningManagementSystem.Admin
{
    ...
    public partial class AssignStudentSubject : BasePage
    {
        ...
        void LoadSubjects()
        {
            if (!TryGetFilters(out int stream, out int course, out int level, out int sem))
                return;

            gvSubjects.DataSource = bl.GetSubjects(
                InstituteId,
                SessionId,
                stream,
                course,
        ...
        protected void btnLoadSubjects_Click(object sender, EventArgs e)
        {
            if (!TryGetFilters(out int stream, out int course, out int level, out int sem))
            {
                lblMsg.Text = "Please select required filters.";
                lblMsg.CssClass = "alert alert-warning";
                return;
            }
        ...
        protected void btnAssign_Click(object sender, EventArgs e)
        {
            ...
            if (sub.Rows.Count == 0)
            {
                lblMsg.Text = "Select at least one subject.";
                lblMsg.CssClass = "alert alert-warning";
                return;
            }
            ...
            if (!TryGetFilters(out int stream, out int course, out int level, out int sem))
            {
                lblMsg.Text = "Invalid filter selection.";
                return;
            }
        ...
        {
            gvAssigned.DataSource =
                bl.GetAssigned(InstituteId, SessionId);
        ...
        // ✅ CENTRAL SAFE FILTER METHOD (VERY IMPORTANT)
        private bool TryGetFilters(out int stream, out int course, out int level, out int sem)
        {
            stream = course = level = sem = 0;

            if (!int.TryParse(ddlStream.SelectedValue, out stream) || stream == 0)
                return false;

            int.TryParse(ddlCourse.SelectedValue, out course);
            int.TryParse(ddlLevel.SelectedValue, out level);
            int.TryParse(ddlSemester.SelectedValue, out sem);

            return level != 0; // minimum requirement
        }
    }
}