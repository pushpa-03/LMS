...
namespace LearningManagementSystem.Admin
{
    public partial class AssignStudentSubject : BasePage
    {
        ...
        private const int SubjPageSize = 5;
        private const int StudPageSize = 5;
        private const int TrackerPageSize = 5;
        ...
        // ── Stored filter IDs (survive postback) ────────────────────
        private int VS_StreamId { get => (int)(ViewState["vs_str"] ?? 0); set => ViewState["vs_str"] = value; }
        private int VS_CourseId { get => (int)(ViewState["vs_crs"] ?? 0); set => ViewState["vs_crs"] = value; }
        private int VS_LevelId { get => (int)(ViewState["vs_lvl"] ?? 0); set => ViewState["vs_lvl"] = value; }
        private int VS_SemId { get => (int)(ViewState["vs_sem"] ?? 0); set => ViewState["vs_sem"] = value; }
        ...
        // ══════════════════════════════════════════════════════════════
        //  PAGE LOAD
        //  KEY FIX: on every postback, if a class was previously loaded
        //  (VS_StreamId > 0), re-bind the subject and student grids so
        //  the pager panels and their Enabled states are restored.
        //  This runs BEFORE any button Click event, which is exactly
        //  what ASP.NET requires — controls must be in their correct
        //  state before the event handler runs.
        // ══════════════════════════════════════════════════════════════
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            ...
            {
                // Re-bind grids from DB so pager panels regain correct Visible/Enabled state.
                // This is safe — it runs before the triggering button's Click event fires.
                if (VS_StreamId > 0 && VS_LevelId > 0 && VS_SemId > 0)
                {
                    ReloadSubjGrid();
                    ReloadStudGrid();
                ...
                // Tracker always needs re-binding on postback so its pager is correct.
                BindTracker();
            }
        }
        ...
        private void ClearGrids()
        {
            ...
            // Hide tracker pager number buttons since no class is loaded
            // (tracker itself is independent, re-bound by Page_Load)
        }
        ...
        protected void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Persist filter context in ViewState
                VS_StreamId = streamId; VS_CourseId = courseId;
                VS_LevelId = levelId; VS_SemId = semId; VS_SecId = secId;
                VS_SubjTotal = dtSubj.Rows.Count;
                VS_StudTotal = dtStud.Rows.Count;

                // Sync hidden fields for Assign button
                hfStreamId.Value = streamId.ToString();
                hfCourseId.Value = courseId.ToString();
                hfLevelId.Value = levelId.ToString();
                hfSemesterId.Value = semId.ToString();
        ...
        //  SUBJECT PAGER
        // ══════════════════════════════════════════════════════════════
        protected void SubjPrev_Click(object sender, EventArgs e)
        {
            SubjPage = Math.Max(1, SubjPage - 1);
            ReloadSubjGrid();
        }
        protected void SubjNext_Click(object sender, EventArgs e)
        {
            SubjPage++;
            ReloadSubjGrid();
        }
        ...
        //  STUDENT PAGER
        // ══════════════════════════════════════════════════════════════
        protected void StudPrev_Click(object sender, EventArgs e)
        {
            StudPage = Math.Max(1, StudPage - 1);
            ReloadStudGrid();
        }
        protected void StudNext_Click(object sender, EventArgs e)
        {
            StudPage++;
            ReloadStudGrid();
        }
        ...
        // ══════════════════════════════════════════════════════════════
        //  TRACKER GRID + STATIC PAGER
        //
        //  Strategy: we have 10 static numbered buttons (btnTrkP1–P10).
        //  BindTracker() calculates a "window" of up to 10 consecutive
        //  page numbers centred on the current page, assigns the correct
        //  page number as Text to each button, and hides buttons that
        //  fall outside the total page count.  The active page button
        ...
        private void BindTracker()
        {
            ...
            catch (Exception ex)
            {
                ...
                gvAssigned.DataSource = null;
                gvAssigned.DataBind();
                pnlTrackerPager.Visible = false;
            }
        }

        // Collect the 10 static numbered buttons into an array for easy indexing.
        private Button[] TrackerPageButtons =>
            new[] { btnTrkP1, btnTrkP2, btnTrkP3, btnTrkP4, btnTrkP5,
                    btnTrkP6, btnTrkP7, btnTrkP8, btnTrkP9, btnTrkP10 };

        private void ApplyTrackerPager(int totalPages, int totalRows)
        {
            // Hide pager entirely when there is only one page.
            pnlTrackerPager.Visible = totalPages > 1;
            if (totalPages <= 1) return;

            // ── Nav buttons ──────────────────────────────────────────
            btnTrkFirst.Enabled = TrackerPage > 1;
            btnTrkPrev.Enabled = TrackerPage > 1;
            btnTrkNext.Enabled = TrackerPage < totalPages;
            btnTrkLast.Enabled = TrackerPage < totalPages;

            // ── Numbered buttons window ──────────────────────────────
            // Show up to 10 pages centred around the current page.
            const int MaxBtns = 10;
            int half = MaxBtns / 2;
            int wStart = Math.Max(1, TrackerPage - half);
            int wEnd = Math.Min(totalPages, wStart + MaxBtns - 1);
            // Adjust window start if window end hit the ceiling.
            wStart = Math.Max(1, wEnd - MaxBtns + 1);

            Button[] btns = TrackerPageButtons;
            for (int i = 0; i < btns.Length; i++)
            {
                int pageNum = wStart + i;
                if (pageNum <= wEnd)
                {
                    btns[i].Text = pageNum.ToString();
                    btns[i].Visible = true;
                    btns[i].Enabled = true;
                    // Active style: highlight current page
                    btns[i].CssClass = pageNum == TrackerPage
                        ? "ass-page-btn active"
                        : "ass-page-btn";
                }
                else
            ...
            // ── Info label ───────────────────────────────────────────
            int rowFrom = (TrackerPage - 1) * TrackerPageSize + 1;
            int rowTo = Math.Min(TrackerPage * TrackerPageSize, totalRows);
            lblTrackerPage.Text = $"{rowFrom}–{rowTo} of {totalRows}";
        }

        // ── Tracker pager navigation click handlers ─────────────────

        protected void TrkFirst_Click(object sender, EventArgs e)
        {
            TrackerPage = 1;
            BindTracker();
        }

        protected void TrkPrev_Click(object sender, EventArgs e)
        {
            TrackerPage = Math.Max(1, TrackerPage - 1);
            BindTracker();
        }

        protected void TrkNext_Click(object sender, EventArgs e)
        {
            TrackerPage++;
            BindTracker();
        }

        protected void TrkLast_Click(object sender, EventArgs e)
        {
            // We don't store totalPages here, but BindTracker() will clamp to max.
            TrackerPage = int.MaxValue;
            BindTracker();
        }

        // Shared handler for all 10 numbered buttons — reads the button Text as target page.
        protected void TrkPage_Click(object sender, EventArgs e)
        {
            var btn = sender as Button;
            if (btn != null && int.TryParse(btn.Text, out int p))
            {
                TrackerPage = p;
                BindTracker();
            }
        ...
        protected void gvAssigned_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                // Refresh student grid if a class is loaded
                if (VS_StreamId > 0 && VS_LevelId > 0 && VS_SemId > 0)
                {
                    DataTable dtStud = _bl.GetStudentsWithAssignmentStatus(
                        InstituteId, SessionId, VS_StreamId, VS_CourseId, VS_LevelId, VS_SemId, VS_SecId);
