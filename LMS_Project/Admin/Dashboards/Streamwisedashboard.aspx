...
<style>
/* ══════════════════════════════════════════════════
   STREAM WISE DASHBOARD — FULL RESPONSIVE REWRITE
   Tokens kept identical · only layout rules updated
══════════════════════════════════════════════════ */
:root{
  --p:#4f46e5;--pl:#ede9fe;--pd:#3730a3;
  --g:#10b981;--gl:#d1fae5;
...
.tb-left h1{
  font-size:clamp(17px,3vw,21px);font-weight:800;
  display:flex;align-items:center;gap:10px;
  flex-wrap:wrap;
}
.h1-ico{
  width:36px;height:36px;background:var(--pl);color:var(--p);
  border-radius:10px;display:flex;align-items:center;justify-content:center;
  font-size:15px;flex-shrink:0;
}
.tb-left p{
  font-size:13px;color:var(--ts);margin-top:3px;
  padding-left:46px;word-break:break-word;
}
...
.ddl{
  ...
  font-weight:500;width:100%;min-width:160px;max-width:280px;
...
.banner{
  ...
  background:linear-gradient(135deg,#1a1560 0%,#3b30c8 45%,#7033d4 100%);
  box-shadow:var(--shm);
}
.b-bgimg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;display:block;}
.b-ov{position:absolute;inset:0;background:linear-gradient(100deg,rgba(8,5,55,.78),rgba(8,5,55,.22));z-index:1;}
.b-body{
...
.b-title{
  font-size:clamp(16px,3.5vw,24px);font-weight:800;color:#fff;
  margin-bottom:4px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;
}
.b-title i{opacity:.85;}
.b-sub{font-size:13px;color:rgba(255,255,255,.72);}
.b-stats{display:flex;gap:14px;margin-top:14px;flex-wrap:wrap;}
.bstat .v{font-size:clamp(16px,3vw,22px);font-weight:900;color:#fff;}
.bstat .l{font-size:10px;color:rgba(255,255,255,.62);text-transform:uppercase;letter-spacing:.04em;}

/* Upload column */
.up-col{display:flex;flex-direction:column;align-items:flex-end;gap:8px;min-width:180px;flex-shrink:0;}
.dz{
  width:100%;border:2px dashed rgba(255,255,255,.4);border-radius:10px;
  padding:14px 8px;text-align:center;cursor:pointer;
  transition:.2s;background:rgba(255,255,255,.07);
}
.dz:hover{border-color:#fff;background:rgba(255,255,255,.16);}
.dz i{font-size:24px;color:#fff;display:block;margin-bottom:5px;}
.dz-lbl{font-size:11px;color:rgba(255,255,255,.82);line-height:1.5;}
.btn-up,.btn-rm{
  width:100%;padding:8px 0;border-radius:var(--rads);
  font-size:12px;font-weight:700;cursor:pointer;transition:.15s;border:none;
}
.btn-up{background:#fff;color:var(--p);}
.btn-up:hover{background:#f0efff;transform:translateY(-1px);}
.btn-rm{background:rgba(255,255,255,.14);color:#fff;border:1px solid rgba(255,255,255,.32);}
.btn-rm:hover{background:rgba(255,255,255,.26);}
#upProg{width:100%;height:4px;background:rgba(255,255,255,.2);border-radius:2px;overflow:hidden;}
#upBar{height:4px;background:#fff;width:0%;transition:width .35s;border-radius:2px;}
...
.kpi-lbl{font-size:10px;font-weight:700;color:var(--ts);text-transform:uppercase;letter-spacing:.05em;}
.kpi-ico{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:17px;}
.ic-b{background:var(--bl);color:var(--b);}  .ic-g{background:var(--gl);color:var(--g);}
.ic-pu{background:var(--pul);color:var(--pu);} .ic-w{background:var(--wl);color:var(--w);}
.ic-t{background:var(--tl);color:var(--t);}   .ic-ro{background:var(--rol);color:var(--ro);}
.kpi-val{font-size:clamp(22px,4vw,30px);font-weight:900;color:var(--tx);line-height:1;}
.kpi-hint{font-size:11px;color:var(--tm);margin-top:4px;}
...
.sc{
  ...
  padding:14px 16px;min-width:162px;flex-shrink:0;box-shadow:var(--sh);
  transition:.18s;cursor:pointer;
}
.sc:hover,.sc.act{border-color:var(--p);box-shadow:0 0 0 3px rgba(79,70,229,.13);}
.sc-name{font-size:13px;font-weight:700;color:var(--tx);margin-bottom:8px;
  display:flex;align-items:center;gap:6px;}
.sc-name i{color:var(--p);font-size:11px;}
.sc-stat{display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;}
.sc-stat span:first-child{color:var(--ts);}
.sc-stat span:last-child{font-weight:700;}
.att-track{height:5px;background:var(--bd);border-radius:99px;margin-top:8px;overflow:hidden;}
.att-fill{height:5px;background:var(--g);border-radius:99px;width:0%;transition:width 1.2s ease;}
.att-lbl{font-size:10px;color:var(--ts);text-align:right;margin-top:3px;}
...
.card{
  background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad);
  box-shadow:var(--sh);padding:18px;transition:box-shadow .18s;
  min-width:0;/* prevent overflow in grid */
}
.card:hover{box-shadow:var(--shm);}
.card-hd{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;gap:10px;flex-wrap:wrap;}
.card-hd-l{display:flex;align-items:center;gap:10px;}
.c-ico{width:34px;height:34px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;}
.c-title{font-size:13px;font-weight:700;color:var(--tx);}
.c-sub{font-size:11px;color:var(--ts);margin-top:2px;}
.chart-box{position:relative;width:100%;}

/* ══ GRID LAYOUTS — fluid, no fixed 2-col on mobile ══ */
/* Desktop first */
...
.tbl th{
  ...
  letter-spacing:.04em;padding:0 12px 10px;border-bottom:2px solid var(--bd);
...
.num{font-size:11px;color:var(--tm);}
.sname{font-weight:600;color:var(--tx);}
.scode{display:inline-block;font-size:10px;background:var(--pl);color:var(--p);padding:2px 8px;border-radius:99px;font-weight:700;}
...
/* ══ TEACHERS LIST ═══════════════════════════════ */
.tch{display:flex;align-items:flex-start;gap:12px;padding:12px 0;border-bottom:1px solid var(--bd);flex-wrap:wrap;}
.tch:last-child{border:none;}
.av{
  width:42px;height:42px;border-radius:50%;background:var(--pl);color:var(--p);
...
.tch-name{font-size:13px;font-weight:700;color:var(--tx);}
.tch-des{font-size:11px;color:var(--ts);margin-top:1px;}
.tch-chips{display:flex;gap:5px;margin-top:5px;flex-wrap:wrap;}
.tch-right{margin-left:auto;text-align:right;flex-shrink:0;}
.tch-right .big{font-size:16px;font-weight:800;color:var(--p);}
.tch-right .sm{font-size:10px;color:var(--tm);}
...
/* ══ PROGRESS BARS ═══════════════════════════════ */
.pi{margin-bottom:14px;}
.pi-lbl{display:flex;justify-content:space-between;font-size:12px;font-weight:500;color:var(--tx);margin-bottom:5px;flex-wrap:wrap;gap:4px;}
.pi-lbl span:last-child{color:var(--ts);}
.pi-track{height:8px;background:var(--bd);border-radius:99px;overflow:hidden;}
.pi-fill{height:8px;border-radius:99px;transition:width 1.1s ease;width:0%;}
...
.empty i{font-size:28px;display:block;margin-bottom:8px;opacity:.45;}
.empty p{font-size:13px;}
...
@media(max-width:900px){
  ...
  .tb-right{flex-direction:column;align-items:flex-start;}
  .ddl{max-width:100%;}
...
@media(max-width:700px){
  ...
  .b-stats{gap:10px;margin-top:10px;}
  .bstat .v{font-size:17px;}
  .up-col{align-items:flex-start;min-width:0;width:100%;}
  .dz,.btn-up,.btn-rm,#upProg{max-width:100%;}
  .chart-box{/* let chart fill container */}
  .card{padding:14px;}
  .card-hd{margin-bottom:12px;}
  .tch{flex-wrap:nowrap;}/* keep avatar + name on same line */
}
...
@media(max-width:480px){
  ...
  .sess-badge{font-size:11px;padding:6px 12px;}
  .b-body{flex-direction:column;gap:14px;}
  .b-stats{flex-wrap:wrap;gap:8px;}
  .bstat .v{font-size:16px;}
  .sc{min-width:148px;}
  .tch{flex-wrap:wrap;}
  .tch-right{margin-left:0;text-align:left;}
}
...
@media(max-width:360px){
  ...
  .sc{min-width:138px;padding:12px 12px;}
}
</style>
</asp:Content>
...
<%-- Hidden fields for chart data — UNCHANGED --%>
<asp:HiddenField ID="hdnCourseLabels"    runat="server"/>
<asp:HiddenField ID="hdnCourseStudents"  runat="server"/>
<asp:HiddenField ID="hdnTrendMonths"     runat="server"/>
<asp:HiddenField ID="hdnTrendStudents"   runat="server"/>
...
<div class="wrap">
  ...
  <%-- ROW 1: Enrollment trend + 7-day attendance — UNCHANGED --%>
  <div class="g21">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ROW 2: Course + Gender + Quiz — UNCHANGED --%>
  <div class="g3">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ROW 3: Subject table + Teachers — UNCHANGED (table wrapped) --%>
  <div class="g21">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ROW 4: Assignment progress bars + Polar chart — UNCHANGED --%>
  <div class="g2">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
    ...
    (function () {
        ...
        (function(){
            ...
            new Chart(ctx,{type:'line',data:{labels:lab,datasets:[{label:'Attendance %',data:dat,
                ...
                options:{responsive:true,maintainAspectRatio:false,
                plugins:{...noLeg,tooltip:{...TT,callbacks:{label:c=>' '+c.raw+'%'}}},
                scales:{x:{grid:{display:false},ticks:TICK},y:{beginAtZero:true,max:100,grid:GRD,ticks:{...TICK,callback:v=>v+'%'}}},animation:ANIM}});
        })();
        ...
        (function(){
            ...
            new Chart(document.getElementById('chartQuiz'),{type:'bar',
                ...
                options:{responsive:true,maintainAspectRatio:false,
                plugins:{legend:{position:'top',labels:{boxWidth:10,font:{size:11}}},tooltip:TT},
                scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{beginAtZero:true,max:100,grid:GRD,ticks:{...TICK,callback:v=>v+'%'}}},animation:ANIM}});
        })();
        ...
        (function(){
            const sl=document.getElementById('<%= lblStudents.ClientID %>'),bl=document.getElementById('<%= lblBStudents.ClientID %>');
            if(sl&&bl)bl.innerText=sl.innerText;
            const tl=document.getElementById('<%= lblTeachers.ClientID %>'),btl=document.getElementById('<%= lblBTeachers.ClientID %>');
            if(tl&&btl)btl.innerText=tl.innerText;
            const cl=document.getElementById('<%= lblCourses.ClientID %>'),bcl=document.getElementById('<%= lblBCourses.ClientID %>');
            if(cl&&bcl)bcl.innerText=cl.innerText;
            const al=document.getElementById('<%= lblAttPct.ClientID %>'),bal=document.getElementById('<%= lblBAtt.ClientID %>');
            if (al && bal) bal.innerText = al.innerText;
        })();
