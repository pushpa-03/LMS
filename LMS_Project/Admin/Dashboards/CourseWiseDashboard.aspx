...
<style>
/* ══════════════════════════════════════════════════════════
   COURSE WISE DASHBOARD — FULL RESPONSIVE
   All design tokens identical · Only layout / sizing updated
══════════════════════════════════════════════════════════ */
:root{
  --p:#4f46e5;--pl:#ede9fe;--pd:#3730a3;
  --g:#10b981;--gl:#d1fae5;--gd:#059669;
...
.dash-alert{
  display:flex;align-items:center;gap:10px;
  padding:12px 16px;border-radius:var(--rads);
  font-size:13px;font-weight:500;margin-bottom:18px;
  animation:fadeDown .3s ease;flex-wrap:wrap;
}
@keyframes fadeDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:none}}
.alert-success{background:var(--gl);color:#065f46;border-left:3px solid var(--g);}
.alert-danger {background:var(--rl);color:#991b1b;border-left:3px solid var(--r);}
...
.tb-left p{
  font-size:13px;color:var(--ts);margin-top:3px;
  padding-left:48px;word-break:break-word;
}
.tb-right{display:flex;align-items:center;gap:10px;flex-wrap:wrap;width:100%;}
.ddl{
  padding:9px 14px;border:1.5px solid var(--bd);border-radius:var(--rads);
  font-size:13px;color:var(--tx);background:#fff;cursor:pointer;
  font-weight:500;width:100%;min-width:160px;max-width:300px;
  transition:border-color .2s,box-shadow .2s;
}
.ddl:focus{border-color:var(--p);outline:none;box-shadow:0 0 0 3px rgba(79,70,229,.12);}
.sess-pill{
  background:var(--pl);color:var(--p);padding:7px 14px;
  border-radius:20px;font-size:12px;font-weight:700;
...
.b-title{
  font-size:clamp(15px,3.5vw,24px);font-weight:800;
  color:#fff;margin-bottom:4px;line-height:1.2;word-break:break-word;
}
.b-sub{font-size:13px;color:rgba(255,255,255,.70);}
.b-kpis{display:flex;gap:clamp(10px,2vw,20px);margin-top:14px;flex-wrap:wrap;align-items:center;}
.bk{text-align:center;min-width:48px;}
.bk-v{font-size:clamp(16px,3vw,22px);font-weight:900;color:#fff;line-height:1;}
.bk-l{font-size:10px;color:rgba(255,255,255,.58);text-transform:uppercase;letter-spacing:.05em;margin-top:2px;}
.b-divider{width:1px;background:rgba(255,255,255,.25);align-self:stretch;margin:0 2px;min-height:32px;}
...
.kpi{
  background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad);
  padding:16px 14px;box-shadow:var(--sh);position:relative;overflow:hidden;
  transition:transform .18s,box-shadow .18s;cursor:default;min-width:0;
}
.kpi:hover{transform:translateY(-3px);box-shadow:var(--shm);}
.kpi::before{
  content:'';position:absolute;top:0;left:0;right:0;height:3px;
...
.i-t {background:var(--tl);color:var(--t);}   .i-ro{background:var(--rol);color:var(--ro);}
.i-cy{background:var(--cyl);color:var(--cy);}
.kpi-val{font-size:clamp(20px,3.5vw,28px);font-weight:900;color:var(--tx);line-height:1;letter-spacing:-.5px;}
.kpi-sub{font-size:11px;color:var(--tm);margin-top:4px;}
...
.cc{
  background:var(--bg);border:1.5px solid var(--bd);border-radius:var(--rad);
  padding:14px 16px;min-width:168px;flex-shrink:0;box-shadow:var(--sh);
  transition:border-color .18s,box-shadow .18s;cursor:pointer;
}
.cc:hover{border-color:var(--p);box-shadow:0 0 0 3px rgba(79,70,229,.12);}
.cc-name{font-size:13px;font-weight:700;color:var(--tx);margin-bottom:4px;}
.cc-stream{font-size:11px;color:var(--ts);margin-bottom:9px;}
.cc-row{display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;}
.cc-row span:first-child{color:var(--ts);}
.cc-row span:last-child{font-weight:700;color:var(--tx);}
.att-bar{height:5px;background:var(--bd);border-radius:99px;margin-top:9px;overflow:hidden;}
.att-fill{height:5px;background:var(--g);border-radius:99px;width:0%;transition:width 1.2s ease;}
.att-lbl{font-size:10px;color:var(--ts);text-align:right;margin-top:3px;}
...
.card{
  background:var(--bg);border:1px solid var(--bd);border-radius:var(--rad);
  box-shadow:var(--sh);padding:18px;transition:box-shadow .18s;min-width:0;
}
.card:hover{box-shadow:var(--shm);}
.card-hd{
  display:flex;align-items:flex-start;justify-content:space-between;
...
.c-title{font-size:13px;font-weight:700;color:var(--tx);}
.c-sub{font-size:11px;color:var(--ts);margin-top:2px;}
.chart-box{position:relative;width:100%;}

/* ══ GRID LAYOUTS ══════════════════════════════════════════ */
/* All grids collapse properly — no fixed cols on small screens */
...
.tbl th{
  font-size:10px;font-weight:700;color:var(--ts);text-transform:uppercase;
  letter-spacing:.05em;padding:0 12px 10px;border-bottom:2px solid var(--bd);
  text-align:left;white-space:nowrap;
}
...
.stu-item:last-child{border-bottom:none;}
.stu-item:hover{background:#f8f9ff;margin:0 -18px;padding-left:18px;padding-right:18px;}
.stu-rank{
  width:24px;height:24px;border-radius:50%;
  display:flex;align-items:center;justify-content:center;
  font-size:11px;font-weight:800;flex-shrink:0;
...
.stu-sem{font-size:10px;color:var(--tm);margin-top:1px;}
.stu-right{margin-left:auto;text-align:right;flex-shrink:0;}
.grade-a{background:#d1fae5;color:#065f46;padding:3px 9px;border-radius:99px;font-size:12px;font-weight:800;}
.grade-b{background:#dbeafe;color:#1e40af;padding:3px 9px;border-radius:99px;font-size:12px;font-weight:800;}
.grade-c{background:#fef3c7;color:#92400e;padding:3px 9px;border-radius:99px;font-size:12px;font-weight:800;}
.grade-f{background:#fee2e2;color:#991b1b;padding:3px 9px;border-radius:99px;font-size:12px;font-weight:800;}
.grade-none{background:var(--pg);color:var(--ts);padding:3px 9px;border-radius:99px;font-size:12px;font-weight:700;}
.stu-att{font-size:11px;color:var(--ts);margin-top:3px;}
...
.quiz-score{font-size:14px;font-weight:800;color:var(--p);}
.quiz-pass{font-size:10px;color:var(--ts);margin-top:1px;}
.pass-pill{display:inline-block;font-size:10px;font-weight:700;padding:2px 8px;border-radius:99px;}
.pp-g{background:var(--gl);color:#065f46;}
.pp-w{background:var(--wl);color:#92400e;}
.pp-r{background:var(--rl);color:#991b1b;}
...
@media(max-width:960px){
  ...
  .b-kpis { gap:12px; }
  .b-divider { display:none; } /* hide on tablet — too cramped */
  .bk { text-align:left; }
}
...
@media(max-width:768px){
  ...
  .tb-left p { padding-left:0; margin-top:4px; }
  .ddl { max-width:100%; }
  .banner { min-height:auto; }
  .b-body { padding:16px; flex-direction:column; }
  .b-title { font-size:16px; }
  .b-kpis { flex-wrap:wrap; gap:8px; margin-top:10px; }
  .bk-v { font-size:16px; }
  .card { padding:14px; }
  .card-hd { margin-bottom:12px; }
  .stu-item:hover { margin:0; padding:10px 0; background:transparent; } /* disable hover indent on mobile */
}

/* ── Phone landscape / small phones: ≤ 540px ── */
@media(max-width:540px){
  .kpi-grid { grid-template-columns:repeat(2,1fr); }
  .kpi-val  { font-size:20px; }
  .cc       { min-width:152px; padding:12px 13px; }
  .tb-left h1 { font-size:16px; }
  .b-body   { padding:14px; }
  .quiz-item { flex-wrap:nowrap; }
  .stu-right { margin-left:auto; text-align:right; }
}

/* ── Portrait phone: ≤ 400px ── */
@media(max-width:400px){
  .kpi-grid { grid-template-columns:1fr 1fr; }
  .kpi      { padding:13px 11px; }
  .kpi-lbl  { font-size:9px; }
  .kpi-val  { font-size:18px; }
  .kpi-ico  { width:32px; height:32px; font-size:13px; }
  .sess-pill { font-size:11px; padding:6px 10px; }
  .cc       { min-width:140px; }
...
/* ── Ensure charts never overflow their card on tiny screens ── */
@media(max-width:480px){
  [style*="height:250px"] { height:210px !important; }
  [style*="height:245px"] { height:210px !important; }
  [style*="height:240px"] { height:200px !important; }
  [style*="height:220px"] { height:190px !important; }
  [style*="height:215px"] { height:185px !important; }
  [style*="height:175px"] { height:160px !important; }
}
</style>
</asp:Content>
...
<div class="wrap">
  ...
  <div class="kpi-grid">
    <div class="kpi kc-b">
      <div class="kpi-top"><span class="kpi-lbl">Total Students</span><div class="kpi-ico i-b"><i class="fa fa-users"></i></div></div>
      <div class="kpi-val" id="kStudents">0</div>
      <div class="kpi-sub">Enrolled this session</div>
    </div>
    <div class="kpi kc-g">
      <div class="kpi-top"><span class="kpi-lbl">Attendance</span><div class="kpi-ico i-g"><i class="fa fa-calendar-check"></i></div></div>
      <div class="kpi-val" id="kAtt">0%</div>
      <div class="kpi-sub">Course average</div>
    </div>
    <div class="kpi kc-pu">
      <div class="kpi-top"><span class="kpi-lbl">Subjects</span><div class="kpi-ico i-pu"><i class="fa fa-book-open"></i></div></div>
      <div class="kpi-val" id="kSubj">0</div>
      <div class="kpi-sub">Mapped to course</div>
    </div>
    <div class="kpi kc-w">
      <div class="kpi-top"><span class="kpi-lbl">Sections</span><div class="kpi-ico i-w"><i class="fa fa-sitemap"></i></div></div>
      <div class="kpi-val" id="kSec">0</div>
      <div class="kpi-sub">Active sections</div>
    </div>
    <div class="kpi kc-t">
      <div class="kpi-top"><span class="kpi-lbl">Assignments Sub.</span><div class="kpi-ico i-t"><i class="fa fa-clipboard-check"></i></div></div>
      <div class="kpi-val" id="kAssign">0</div>
      <div class="kpi-sub"><asp:Label ID="lblAssignSub" runat="server" Text="0"/></div>
    </div>
    <div class="kpi kc-ro">
      <div class="kpi-top"><span class="kpi-lbl">Quiz Attempts</span><div class="kpi-ico i-ro"><i class="fa fa-circle-question"></i></div></div>
      <div class="kpi-val" id="kQuiz">0</div>
      <div class="kpi-sub"><asp:Label ID="lblQuizAttempts" runat="server" Text="0"/></div>
    </div>
    <div class="kpi kc-cy">
      <div class="kpi-top"><span class="kpi-lbl">Avg Quiz Score</span><div class="kpi-ico i-cy"><i class="fa fa-chart-simple"></i></div></div>
      <div class="kpi-val" id="kAvgQ">0</div>
      <div class="kpi-sub">Out of 100</div>
    </div>
  </div>

  <%-- ══ COURSE MINI-CARDS SCROLL — UNCHANGED ══ --%>
  <div class="course-scroll">
    <asp:Repeater ID="rptCourseCards" runat="server">
      <ItemTemplate>
        <div class="cc">
  ...
  <%-- ══ ROW 1: Enrollment Trend + Attendance by Week — UNCHANGED ══ --%>
  <div class="g21">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ══ ROW 2: Level + Semester + Gender — UNCHANGED ══ --%>
  <div class="g3">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ══ ROW 3: Subject Performance + Section pie — UNCHANGED ══ --%>
  <div class="g21">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ══ ROW 4: Subject Avg Score + Assignment rate — UNCHANGED ══ --%>
  <div class="g2">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
  ...
  <%-- ══ ROW 5: Quiz + Top students — UNCHANGED ══ --%>
  <div class="g12">
    <div class="card">
      <div class="card-hd">
        <div class="card-hd-l">
      ...
      <asp:Repeater ID="rptQuizzes" runat="server">
        ...
        <FooterTemplate>
          <%# rptQuizzes.Items.Count == 0 ? "<div class='empty'><i class='fa fa-circle-question'></i><p>No quiz data found</p></div>" : "" %>
        </FooterTemplate>
      </asp:Repeater>
    </div>
    ...
    <div class="card">
      ...
      <asp:Repeater ID="rptTopStudents" runat="server">
        ...
        <FooterTemplate>
          <%# rptTopStudents.Items.Count == 0 ? "<div class='empty'><i class='fa fa-users'></i><p>No student data found</p></div>" : "" %>
        </FooterTemplate>
      </asp:Repeater>
    </div>
  </div>

  <%-- ══ ROW 6: Subject table — added tbl-scroll wrapper for mobile ══ --%>
  <div class="card" style="margin-bottom:20px;">
    <div class="card-hd">
      <div class="card-hd-l">
        <div class="c-ico" style="background:var(--bl);color:var(--b);"><i class="fa fa-table-list"></i></div>
          ...
          <asp:Repeater ID="rptSubjects" runat="server">
            <ItemTemplate>
              <tr>
                <td class="snum"><%# Container.ItemIndex + 1 %></td>
                <td class="sname"><%# Server.HtmlEncode(Eval("SubjectName").ToString()) %></td>
                <td><span class="scode"><%# string.IsNullOrWhiteSpace(Eval("SubjectCode")?.ToString()) ? "—" : Server.HtmlEncode(Eval("SubjectCode").ToString()) %></span></td>
                <td><span class="chip cv"><i class="fa fa-video"></i><%# Eval("Videos") %></span></td>
                <td><span class="chip ca"><i class="fa fa-clipboard"></i><%# Eval("Assignments") %></span></td>
                <td><span class="chip cq"><i class="fa fa-circle-question"></i><%# Eval("Quizzes") %></span></td>
                <td>
                ...
                <td><span class="chip cat"><i class="fa fa-calendar-check"></i><%# FmtDec(Eval("AttPct")) %>%</span></td>
              </tr>
            </ItemTemplate>
            <FooterTemplate>
              <%# rptSubjects.Items.Count == 0
...
<script>
    (function () {
        ...
        (function () {
            ...
            const ctx  = document.getElementById('chartTrend').getContext('2d');
            const grad = gradV(ctx, 230, 'rgba(79,70,229,.30)', 'rgba(79,70,229,.01)');
            new Chart(ctx, {type:'line',data:{labels:months,datasets:[{label:'New Students',data,borderColor:'#4f46e5',backgroundColor:grad,borderWidth:2.5,tension:.42,fill:true,pointBackgroundColor:'#4f46e5',pointRadius:5,pointHoverRadius:8,pointHoverBackgroundColor:'#fff',pointHoverBorderColor:'#4f46e5',pointHoverBorderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{...noLeg,tooltip:{...TT,callbacks:{label:c=>' '+c.raw+' students'}}},scales:{x:{grid:{display:false},ticks:TICK},y:{beginAtZero:true,grid:GRD,ticks:TICK}},animation:ANIM}});
        })();
        ...
        (function () {
            ...
            const absent  = hf('<%= hdnAttAbsent.ClientID %>');
            if (!weeks.length) { empty('chartAttWeek','No attendance data'); return; }
            new Chart(document.getElementById('chartAttWeek'),{type:'bar',data:{labels:weeks,datasets:[{label:'Present',data:present,backgroundColor:'#10b981',borderRadius:4,stack:'s'},{label:'Absent',data:absent,backgroundColor:'#ef4444',borderRadius:4,stack:'s'}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{boxWidth:10,font:{size:11}}},tooltip:{...TT}},scales:{x:{grid:{display:false},ticks:TICK,stacked:true},y:{grid:GRD,ticks:TICK,stacked:true}},animation:ANIM}});
        })();
        ...
        (function () {
            ...
            const data   = hf('<%= hdnLevelStudents.ClientID %>');
            if (!labels.length) { empty('chartLevel','No level data'); return; }
            new Chart(document.getElementById('chartLevel'),{type:'bar',data:{labels,datasets:[{label:'Students',data,backgroundColor:palA(.82),borderRadius:6,borderSkipped:false}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{...noLeg,tooltip:TT},scales:{x:{beginAtZero:true,grid:GRD,ticks:TICK},y:{grid:{display:false},ticks:{font:{size:11}}}},animation:ANIM}});
        })();
        ...
        (function () {
            ...
            const data   = hf('<%= hdnSemStudents.ClientID %>');
            if (!labels.length) { empty('chartSem','No semester data'); return; }
            new Chart(document.getElementById('chartSem'),{type:'doughnut',data:{labels,datasets:[{data,backgroundColor:palA(.85),borderWidth:2,borderColor:'#fff',hoverOffset:8}]},options:{cutout:'58%',responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{boxWidth:10,font:{size:11}}},tooltip:TT},animation:{animateRotate:true,duration:1100}}});
        })();
        ...
        (function () {
            ...
            if (!labels.length) { empty('chartGender','No gender data'); return; }
            const GCOL = ['#4f46e5','#f43f5e','#10b981','#f59e0b','#3b82f6'];
            new Chart(document.getElementById('chartGender'),{type:'doughnut',data:{labels,datasets:[{data,backgroundColor:GCOL,borderWidth:3,borderColor:'#fff',hoverOffset:8}]},options:{cutout:'65%',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:TT},animation:{animateRotate:true,duration:1100}}});
            const total = data.reduce((a,b)=>a+b,0)||1;
            const leg = document.getElementById('gLeg');
            labels.forEach((l,i)=>{leg.innerHTML+=`<div style="display:flex;align-items:center;gap:5px;font-size:12px;"><span style="width:10px;height:10px;border-radius:2px;background:${GCOL[i]};display:inline-block;flex-shrink:0;"></span>${l} <strong style="color:${GCOL[i]};">${Math.round(data[i]/total*100)}%</strong></div>`;});
        })();
        ...
        (function () {
            ...
            if (!labels.length) { empty('chartSubjContent','No subject data'); return; }
            const short = labels.map(l=>l.length>14?l.substring(0,13)+'…':l);
            new Chart(document.getElementById('chartSubjContent'),{type:'bar',data:{labels:short,datasets:[{label:'Videos',data:videos,backgroundColor:'rgba(59,130,246,.82)',borderRadius:5},{label:'Assignments',data:assigns,backgroundColor:'rgba(245,158,11,.82)',borderRadius:5},{label:'Quizzes',data:quizzes,backgroundColor:'rgba(139,92,246,.82)',borderRadius:5}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{boxWidth:10,font:{size:11}}},tooltip:TT},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{beginAtZero:true,grid:GRD,ticks:TICK}},animation:ANIM}});
        })();
        ...
        (function () {
            ...
            const data   = hf('<%= hdnSecStudents.ClientID %>');
            if (!labels.length) { empty('chartSec','No section data'); return; }
            new Chart(document.getElementById('chartSec'),{type:'doughnut',data:{labels,datasets:[{data,backgroundColor:PAL,borderWidth:2,borderColor:'#fff',hoverOffset:8}]},options:{cutout:'60%',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:TT},animation:{animateRotate:true,duration:1100}}});
            const total=data.reduce((a,b)=>a+b,0)||1;
            const leg=document.getElementById('secLeg');
            labels.forEach((l,i)=>{leg.innerHTML+=`<div style="display:flex;align-items:center;gap:5px;font-size:12px;"><span style="width:10px;height:10px;border-radius:2px;background:${PAL[i%PAL.length]};display:inline-block;flex-shrink:0;"></span>${l} <strong style="color:${PAL[i%PAL.length]};">${Math.round(data[i]/total*100)}%</strong></div>`;});
        })();
        ...
        (function () {
            ...
            if (!labels.length) { empty('chartSubjPerf','No performance data'); return; }
            const short = labels.map(l=>l.length>12?l.substring(0,11)+'…':l);
            new Chart(document.getElementById('chartSubjPerf'),{type:'bar',data:{labels:short,datasets:[{label:'Avg Score',data:scores,backgroundColor:'rgba(79,70,229,.80)',borderRadius:5,yAxisID:'y'},{label:'Attendance %',data:att,type:'line',borderColor:'#10b981',backgroundColor:'rgba(16,185,129,.10)',borderWidth:2.5,tension:.4,fill:true,pointRadius:4,pointHoverRadius:7,pointBackgroundColor:'#10b981',pointHoverBackgroundColor:'#fff',yAxisID:'y1'}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{position:'top',labels:{boxWidth:10,font:{size:11}}},tooltip:TT},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{position:'left',beginAtZero:true,max:100,grid:GRD,ticks:TICK,title:{display:true,text:'Score',font:{size:10}}},y1:{position:'right',beginAtZero:true,max:100,grid:{display:false},ticks:{...TICK,callback:v=>v+'%'},title:{display:true,text:'Attendance %',font:{size:10}}}},animation:ANIM}});
        })();
        ...
        (function () {
            ...
            const subs     = hf('<%= hdnAssignSubmitted.ClientID %>');
            const wrap = document.getElementById('assignBars');
            if (!subjects.length) { wrap.innerHTML = "<div class='empty'><i class='fa fa-clipboard-check'></i><p>No assignment data</p></div>"; return; }
            subjects.forEach((s, i) => {
                const pct = Math.min(parseFloat(rates[i]) || 0, 100).toFixed(1);
                const tot = totals[i] || 0, sub = subs[i] || 0;
                const col = pct >= 70 ? '#10b981' : pct >= 40 ? '#f59e0b' : '#ef4444';
                const short = s.length > 22 ? s.substring(0, 21) + '…' : s;
                wrap.innerHTML += `<div class="pi"><div class="pi-lbl"><span title="${s}">${short}</span><span>${sub}/${tot} &nbsp;(${pct}%)</span></div><div class="pi-track"><div class="pi-fill" data-w="${pct}%" style="background:${col};"></div></div></div>`;
            });
            setTimeout(() => document.querySelectorAll('.pi-fill[data-w]').forEach(el => { el.style.width = el.dataset.w; }), 350);
        })();
