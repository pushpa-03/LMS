...
<%-- Radar actual percentage values (0-100) --%>
<asp:HiddenField ID="hfRadarAttPct"    runat="server" />
<asp:HiddenField ID="hfRadarSubPct"    runat="server" />
<asp:HiddenField ID="hfRadarVidPct"    runat="server" />
<asp:HiddenField ID="hfRadarAsgPct"    runat="server" />
<asp:HiddenField ID="hfRadarRatPct"    runat="server" />
...
.topbar{display:flex;align-items:center;justify-content:space-between;padding:18px 0 14px;flex-wrap:wrap;gap:10px}
.back-link{display:inline-flex;align-items:center;gap:7px;font-size:13px;font-weight:600;color:var(--ink4);text-decoration:none;transition:color var(--tr)}
...
.sess-sel{border:1.5px solid var(--surf4);border-radius:var(--rsm);padding:6px 12px;font-size:13px;font-family:var(--f);background:var(--surf);color:var(--ink);cursor:pointer}
.sess-sel:focus{outline:none;border-color:var(--blue)}
.hero{background:var(--surf);border-radius:var(--rlg);border:1.5px solid var(--surf4);box-shadow:var(--sh);padding:30px 32px;display:flex;align-items:flex-start;gap:28px;flex-wrap:wrap;margin-bottom:24px;position:relative;overflow:hidden}
.hero::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,var(--blue),var(--purple),var(--pink))}
.av-ring{width:90px;height:90px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:800;color:#fff;box-shadow:0 0 0 4px rgba(255,255,255,.8),0 0 0 7px currentColor;animation:pop .5s both}
@keyframes pop{0%{transform:scale(.6);opacity:0}80%{transform:scale(1.06)}100%{transform:scale(1);opacity:1}}
.av-img{width:90px;height:90px;border-radius:50%;object-fit:cover;border:4px solid var(--surf4);flex-shrink:0}
...
.pill{display:inline-flex;align-items:center;gap:5px;background:var(--surf3);border-radius:30px;padding:3px 10px;font-size:11px;font-weight:600;color:var(--ink3)}
.status-badge{display:inline-flex;align-items:center;gap:4px;border-radius:30px;padding:3px 10px;font-size:11px;font-weight:700}
...
.kpi{background:var(--surf);border-radius:var(--r);border:1.5px solid var(--surf4);box-shadow:var(--sh);padding:18px 20px;position:relative;overflow:hidden;transition:transform var(--tr),box-shadow var(--tr);animation:fu .5s both}
.kpi:hover{transform:translateY(-4px);box-shadow:var(--shl)}
.kpi::after{content:'';position:absolute;bottom:0;left:0;right:0;height:3px;background:var(--ac,var(--blue));border-radius:0 0 3px 3px}
.kpi-ico{width:36px;height:36px;border-radius:10px;margin-bottom:10px;display:flex;align-items:center;justify-content:center;font-size:15px}
...
.kpi-sub{font-size:10px;color:var(--ink5);margin-top:3px}
@keyframes fu{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
.tabs{display:flex;gap:4px;flex-wrap:wrap;background:var(--surf3);border-radius:12px;padding:5px;margin-bottom:22px;border:1.5px solid var(--surf4)}
.tab-btn{border:none;background:transparent;border-radius:8px;padding:8px 14px;font-family:var(--f);font-size:12px;font-weight:600;color:var(--ink4);cursor:pointer;display:flex;align-items:center;gap:5px;transition:all var(--tr);white-space:nowrap}
...
.card{background:var(--surf);border-radius:var(--r);border:1.5px solid var(--surf4);box-shadow:var(--sh);padding:22px 24px;margin-bottom:18px}
.card-title{font-size:13px;font-weight:700;color:var(--ink3);margin-bottom:14px;display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.card-title .ico{width:28px;height:28px;border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:12px;flex-shrink:0}
...
.pf-l{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--ink5);margin-bottom:3px}
...
.chart-box{background:var(--surf);border-radius:var(--r);border:1.5px solid var(--surf4);box-shadow:var(--sh);padding:20px 22px}
.chart-note{font-size:10px;color:var(--ink5);margin-top:8px;font-style:italic}
canvas{max-height:240px}

/* SUBJECT CARD — grouped with sections inside */
.subj-card{background:var(--surf);border:1.5px solid var(--surf4);border-radius:var(--r);box-shadow:var(--sh);padding:18px 20px;position:relative;overflow:hidden;transition:transform var(--tr),box-shadow var(--tr)}
...
.sc-breadcrumb{font-size:11px;color:var(--ink4);margin-bottom:10px;display:flex;flex-wrap:wrap;align-items:center;gap:4px}
.sc-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:12px}
...
/* Section rows inside subject card */
.sec-list{border-top:1.5px solid var(--surf3);margin-top:10px;padding-top:10px}
.sec-row{display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px dashed var(--surf4)}
.sec-row:last-child{border-bottom:none}
.sec-badge{background:var(--blue-lt);color:var(--blue);border-radius:6px;padding:2px 8px;font-size:10px;font-weight:700;flex-shrink:0;min-width:60px;text-align:center}
.sec-students{font-size:11px;color:var(--ink4);flex-shrink:0;min-width:70px}
.sec-pbar-wrap{flex:1;background:var(--surf3);border-radius:30px;height:6px;overflow:hidden}
.sec-pbar-fill{height:100%;border-radius:30px;background:var(--blue);transition:width 1.2s cubic-bezier(.4,0,.2,1)}
.sec-pct{font-size:10px;font-weight:700;color:var(--blue);flex-shrink:0;min-width:36px;text-align:right}
...
.acc-hd{display:flex;align-items:center;justify-content:space-between;padding:10px 13px;background:var(--surf2);border-radius:var(--rsm);cursor:pointer;border:1px solid var(--surf4);transition:background var(--tr);margin-bottom:3px}
...
.vid-row{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:var(--rsm);border:1px solid var(--surf4);margin-bottom:6px;transition:background var(--tr)}
.vid-row:hover{background:var(--surf2)}
.vid-ico{width:38px;height:38px;border-radius:7px;flex-shrink:0;background:var(--blue-lt);display:flex;align-items:center;justify-content:center;font-size:14px;color:var(--blue)}
.vid-info{flex:1;min-width:0}
.vid-tit{font-size:12px;font-weight:600;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
.tbl thead th{background:linear-gradient(135deg,#4f46e5,#6366f1);color:#fff;padding:10px 13px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;white-space:nowrap;text-align:left}
.tbl tbody td{padding:10px 13px;border-bottom:1px solid var(--surf3);font-size:12px;vertical-align:middle;color:var(--ink2)}
.tbl tbody tr:last-child td{border-bottom:none}
.tbl tbody tr:hover{background:var(--surf2)}
.tag{display:inline-flex;align-items:center;gap:3px;border-radius:6px;padding:2px 8px;font-size:11px;font-weight:700;white-space:nowrap}
...
.tl-dot{position:absolute;left:-24px;top:2px;width:20px;height:20px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;flex-shrink:0}
...
.help-card{background:var(--surf);border:1.5px solid var(--surf4);border-radius:var(--r);padding:14px 16px;margin-bottom:10px}
.help-q{font-size:13px;font-weight:600;color:var(--ink)}
.help-time{font-size:11px;color:var(--ink5);margin:3px 0 8px}
.help-reply{background:var(--blue-lt);border-radius:var(--rsm);padding:9px 12px;border-left:3px solid var(--blue)}
...
.help-pend{background:var(--amber-lt);border-radius:var(--rsm);padding:7px 12px;font-size:12px;color:var(--amber);font-weight:600}
.stat-strip{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px}
.stat-chip{background:var(--surf2);border-radius:9px;border:1.5px solid var(--surf4);padding:12px 18px;flex:1;min-width:90px;text-align:center}
...
.cmt-card{border:1px solid var(--surf4);border-radius:var(--rsm);padding:11px 13px;margin-bottom:7px;transition:background var(--tr)}
.cmt-card:hover{background:var(--surf2)}
.cmt-head{display:flex;align-items:center;gap:8px;margin-bottom:5px;flex-wrap:wrap}
.cmt-av{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff;flex-shrink:0}
...
.tc-row{display:flex;align-items:center;gap:10px;margin-bottom:10px;padding-bottom:10px;border-bottom:1px solid var(--surf4)}
.tc-row:last-child{border-bottom:none;margin-bottom:0;padding-bottom:0}
.tc-num{width:28px;height:28px;border-radius:50%;background:var(--green-lt);color:var(--green);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:11px;flex-shrink:0}
...
.cl-item{display:flex;align-items:center;gap:5px;font-size:11px;color:var(--ink3);font-weight:600}
...
@media(max-width:768px){.hero{padding:20px 16px;gap:16px}.kpi-grid{grid-template-columns:repeat(2,1fr)}.g3{grid-template-columns:1fr}}
@media(max-width:480px){.tab-btn{padding:7px 10px;font-size:11px}.kpi-grid{grid-template-columns:repeat(2,1fr)}}
</style>

<div class="td-root">

<div class="topbar">
    <a href="TeacherList.aspx" class="back-link"><i class="fa fa-arrow-left"></i> Back to Faculty</a>
    <div class="sess-wrap">
        <label><i class="fa fa-calendar-alt me-1"></i>Session:</label>
        <asp:DropDownList ID="ddlSession" runat="server" CssClass="sess-sel" AutoPostBack="true" OnSelectedIndexChanged="ddlSession_Changed" />
    </div>
</div>
...
<div class="hero">
    ...
    <% } else { %>
        <div class="av-ring" style="background:<%=AvatarColor%>;color:<%=AvatarColor%>"><%=TeacherInitials%></div>
    <% } %>
    <div class="hero-info">
    ...
    <div class="hero-right">
        <div style="text-align:right">
            <div style="font-size:10px;font-weight:700;color:var(--ink5);text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">Avg Student Rating</div>
...
<div class="kpi-grid">
    <div class="kpi" style="--ac:var(--blue);animation-delay:.05s">
        ...
        <div class="kpi-sub">subject-section assignments</div>
    </div>
    <div class="kpi" style="--ac:var(--cyan);animation-delay:.10s">
        ...
        <div class="kpi-sub">uploaded this session</div>
    </div>
    <div class="kpi" style="--ac:var(--green);animation-delay:.15s">
        ...
        <div class="kpi-lbl">Total Plays</div>
        <div class="kpi-sub">all roles · raw play counter</div>
    </div>
    <div class="kpi" style="--ac:var(--purple);animation-delay:.20s">
        ...
        <div class="kpi-sub">created this session</div>
    </div>
    <div class="kpi" style="--ac:var(--pink);animation-delay:.25s">
        ...
        <div class="kpi-sub">unique students enrolled</div>
    </div>
    <div class="kpi" style="--ac:var(--amber);animation-delay:.30s">
        ...
        <div class="kpi-sub">students on teacher's videos</div>
    </div>
</div>
...
<div class="tab-pane on" id="tProfile">
    <div class="g2">
        <div class="card">
            <div class="card-title"><span class="ico" style="background:var(--blue-lt);color:var(--blue)"><i class="fa fa-user"></i></span>Personal Information</div>
            <div class="pfg">
        ...
        <div class="card">
            <div class="card-title"><span class="ico" style="background:var(--purple-lt);color:var(--purple)"><i class="fa fa-graduation-cap"></i></span>Academic &amp; Login Details</div>
            <div class="pfg">
                <div class="pf"><div class="pf-l">Username</div><div class="pf-v" style="font-family:var(--mono)"><asp:Label ID="lblUsername" runat="server"/></div></div>
                <div class="pf"><div class="pf-l">Email</div><div class="pf-v"><asp:Label ID="lblEmail" runat="server"/></div></div>
                <div class="pf"><div class="pf-l">Employee ID</div><div class="pf-v" style="font-family:var(--mono);font-weight:700;color:var(--blue)"><asp:Label ID="lblEmpId2" runat="server"/></div></div>
                ...
                <div class="pf"><div class="pf-l">Session</div><div class="pf-v" style="color:var(--blue);font-weight:700"><asp:Label ID="lblSession2" runat="server"/></div></div>
                <div class="pf pf-full"><div class="pf-l">Last Login</div><div class="pf-v"><asp:Label ID="lblLastLogin2" runat="server"/></div></div>
            </div>
        </div>
    </div>

    <%-- Performance Overview Radar — shows REAL % values, no scaling tricks --%>
    <div class="chart-box" style="margin-bottom:18px">
        <div class="card-title">
            <i class="fa fa-chart-area" style="color:var(--blue)"></i>&nbsp;Performance Overview
            <span style="font-size:10px;font-weight:400;color:var(--ink5);margin-left:auto">All axes show % (0–100). Hover for exact value.</span>
        </div>
        <canvas id="radarChart" height="180"></canvas>
        <div class="chart-note">
            Attendance % · Assignment Submission % · Avg Video Rating (×20) · Video Syllabus Completion % · Students Taught (scaled to 100 against institute max)
        </div>
    </div>
</div>

<%-- ════ TAB 2: SUBJECTS — one card per UNIQUE subject, sections listed inside ═══ --%>
<div class="tab-pane" id="tSubjects">
    <div class="chart-row">
        <div class="chart-box">
            <div class="card-title"><i class="fa fa-chart-bar" style="color:var(--blue)"></i>&nbsp;Syllabus Completion % per Subject</div>
            <canvas id="syllabusChart"></canvas>
        </div>
        <div class="chart-box">
            <div class="card-title"><i class="fa fa-users" style="color:var(--green)"></i>&nbsp;Enrolled Students per Subject</div>
        ...
        <asp:Repeater ID="rptSubjects" runat="server" OnItemDataBound="rptSubjects_ItemDataBound">
            <ItemTemplate>
                <div class="subj-card">
                    ...
                    <%-- Breadcrumb: Stream → Course → Level → Semester --%>
                    <div class="sc-breadcrumb">
                        <i class="fa fa-layer-group" style="color:var(--blue)"></i>
                        <span><%# Eval("StreamName") %></span>
                        <i class="fa fa-chevron-right" style="font-size:9px;color:var(--ink5)"></i>
                        <span><%# Eval("CourseName") %></span>
                        <i class="fa fa-chevron-right" style="font-size:9px;color:var(--ink5)"></i>
                        <span><%# Eval("LevelName") %></span>
                        <i class="fa fa-chevron-right" style="font-size:9px;color:var(--ink5)"></i>
                        <span><%# Eval("SemesterName") %></span>
                    </div>

                    <%-- Content counts (same for all sections of this subject) --%>
                    ...
                    <%-- Overall syllabus completion across all sections --%>
                    <div style="margin-bottom:4px">
                        <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px">
                            <span style="font-weight:600;color:var(--ink4)">Overall Syllabus Completion</span>
                    ...
                    <%-- Section-wise breakdown (populated via ItemDataBound) --%>
                    <asp:HiddenField ID="hfSubjectId" runat="server" Value='<%# Eval("SubjectId") %>' />
                    <div class="sec-list" id='<%# "secList_"+Eval("SubjectId") %>'>
                        <div style="font-size:10px;font-weight:700;color:var(--ink4);text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px">
                            <i class="fa fa-sitemap me-1"></i>Section-wise Breakdown
                        </div>
                        <asp:Repeater ID="rptSections" runat="server">
                            <ItemTemplate>
                                <div class="sec-row">
                                    <div class="sec-badge"><%# Eval("SectionName") %></div>
                                    <div class="sec-students"><i class="fa fa-users me-1"></i><%# Eval("SectionStudents") %> students</div>
                                    <div class="sec-pbar-wrap">
                                        <div class="sec-pbar-fill" style="width:<%# Bar(Eval("AvgWatchPct")) %>%"></div>
                                    </div>
                                    <div class="sec-pct"><%# string.Format("{0:F0}%",Eval("AvgWatchPct")) %></div>
                                </div>
                            </ItemTemplate>
                            <FooterTemplate>
                                <asp:PlaceHolder runat="server"
                                    Visible='<%# ((Repeater)Container.NamingContainer).Items.Count == 0 %>'>
                                    <div style="font-size:11px;color:var(--ink5);padding:4px 0">
                                        No section data.
                                    </div>
                                </asp:PlaceHolder>
                            </FooterTemplate>
                    ...
                    <div style="font-size:11px;color:var(--ink4);margin-top:8px">
                        <i class="fa fa-user-graduate me-1"></i><%# Eval("CompletedStudents") %> students completed ≥80%
                    </div>
                </div>
            </ItemTemplate>
            <FooterTemplate>
...
<div class="tab-pane" id="tContent">
    <asp:Repeater ID="rptSubjectChapters" runat="server" OnItemDataBound="rptSubjectChapters_ItemDataBound">
        <ItemTemplate>
                    ...
                    <ItemTemplate>
                        ...
                        <div class="acc-hd" onclick="toggleAcc(this)">
                            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
                                <span style="width:22px;height:22px;border-radius:50%;background:var(--blue-lt);color:var(--blue);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;flex-shrink:0"><%# Container.ItemIndex+1 %></span>
                            ...
                            <div style="display:flex;align-items:center;gap:10px">
                                <span style="font-size:11px;color:var(--ink4)"><i class="fa fa-eye me-1"></i><%# Eval("TotalViews") %> student views</span>
                        ...
                        <div class="acc-body">
                            <asp:Repeater ID="rptChapterVideos" runat="server">
                                <ItemTemplate>
                                    <div class="vid-row">
                                        ...
                                        <div class="vid-stats">
                                            ...
                                            <div class="vid-stat">
                                                <div class="vs-val"><%# string.Format("{0:F1}",Eval("AvgRating")) %><span style="color:var(--gold);font-size:10px">★</span></div>
                                ...
                                <FooterTemplate>
                                    <%# ((Repeater)Container.NamingContainer).Items.Count==0 ? "<div class='empty' style='padding:14px'><i class='fa fa-video'></i><p style='font-size:12px'>No videos.</p></div>" : "" %>
                                </FooterTemplate>
                            </asp:Repeater>
                        </div>
                    </ItemTemplate>
                    <FooterTemplate>
                        <%# ((Repeater)Container.NamingContainer).Items.Count==0 ? "<div class='empty'><i class='fa fa-folder-open'></i><p>No chapters.</p></div>" : "" %>
                    </FooterTemplate>
                </asp:Repeater>
            </div>
        </ItemTemplate>
...
<div class="tab-pane" id="tRatings">
    <div class="chart-row">
        <div class="chart-box">
            <div class="card-title"><i class="fa fa-star" style="color:var(--gold)"></i>&nbsp;Avg Student Rating per Video (top 8)</div>
            <canvas id="ratingChart"></canvas>
        </div>
        <div class="chart-box">
            <div class="card-title"><i class="fa fa-eye" style="color:var(--cyan)"></i>&nbsp;Student Views per Video (top 8)</div>
            <canvas id="viewsChart"></canvas>
        </div>
    </div>
    <div class="card">
        <div class="card-title"><span class="ico" style="background:var(--amber-lt);color:var(--amber)"><i class="fa fa-star"></i></span>All Video Ratings — Students Only</div>
        <div class="tbl-wrap">
            <table class="tbl">
                <thead><tr><th>#</th><th>Video</th><th>Chapter</th><th>Subject</th><th>Avg Rating</th><th>Ratings Count</th><th>Student Views</th></tr></thead>
                <tbody>
                    <asp:Repeater ID="rptRatings" runat="server">
                        <ItemTemplate>
                            <tr>
                                ...
                                <td style="font-weight:600;max-width:200px"><div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:190px" title="<%# Eval("VideoTitle") %>"><%# Eval("VideoTitle") %></div></td>
                                <td><%# Eval("ChapterName") %></td>
                                <td><span class="tag tag-blue"><%# Eval("SubjectName") %></span></td>
                                <td>
                                    <div style="display:flex;align-items:center;gap:6px">
                                        <span style="font-weight:800;color:<%# RatingColor(Eval("AvgRating")) %>"><%# string.Format("{0:F1}",Eval("AvgRating")) %></span>
...
<div class="tab-pane" id="tAssignments">
    <div class="chart-row">
        <div class="chart-box">
            <div class="card-title">
                <i class="fa fa-chart-bar" style="color:var(--purple)"></i>&nbsp;Submission Rate — Top 6 Assignments
            </div>
            <%-- Color legend --%>
            <div class="chart-legend">
                <div class="cl-item"><div class="cl-dot" style="background:rgba(79,70,229,.75)"></div>Submitted (actual count)</div>
                <div class="cl-item"><div class="cl-dot" style="background:rgba(148,163,184,.4)"></div>Total Enrolled Students (target)</div>
            </div>
            <canvas id="asgChart"></canvas>
            <div class="chart-note">
                Purple bar = number of students who submitted. Gray bar = total students enrolled in that subject. Gap between them = pending/missed submissions.
            </div>
        </div>
        <div class="card" style="margin-bottom:0">
            <div class="card-title"><span class="ico" style="background:var(--purple-lt);color:var(--purple)"><i class="fa fa-tasks"></i></span>Assignment Summary</div>
    ...
    <div class="card">
        <div class="card-title"><span class="ico" style="background:var(--blue-lt);color:var(--blue)"><i class="fa fa-file-alt"></i></span>All Assignments</div>
        <div class="tbl-wrap">
            <table class="tbl">
                <thead><tr><th>Assignment</th><th>Subject</th><th>Created</th><th>Due</th><th>Max Marks</th><th>Students</th><th>Submitted</th><th>Graded</th><th>Avg Marks</th><th>Submission %</th></tr></thead>
                <tbody>
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div style="font-size:10px;font-weight:600;color:var(--ink4)"><%# SubRatePct(Eval("Submissions"),Eval("TotalStudents")) %></div>
                                </td>
                            </tr>
                        </ItemTemplate>
                        <FooterTemplate>
...
<div class="tab-pane" id="tAttendance">
    <div class="chart-row">
        <div class="chart-box">
            ...
            <div class="chart-note">Green ≥75% · Amber ≥50% · Red &lt;50%</div>
        </div>
        <div class="card" style="margin-bottom:0">
            <div class="card-title"><span class="ico" style="background:var(--green-lt);color:var(--green)"><i class="fa fa-trophy"></i></span>Top Attending Classes</div>
            <asp:Repeater ID="rptTopClasses" runat="server">
                <ItemTemplate>
                    <div class="tc-row">
                        <div class="tc-num"><%# Container.ItemIndex+1 %></div>
                        <div style="flex:1;min-width:0">
                            <div style="font-size:12px;font-weight:600;color:var(--ink)"><%# Eval("SubjectCode") %> &mdash; <%# Eval("LevelName") %> / <%# Eval("SectionName") %></div>
                            <div style="font-size:10px;color:var(--ink5)"><%# Eval("StudentCount") %> students</div>
                        </div>
                        <div style="font-size:1.1rem;font-weight:800;color:<%# AttColor(Eval("AttendancePct")) %>"><%# string.Format("{0:F1}%",Eval("AttendancePct")) %></div>
                    </div>
                </ItemTemplate>
                <FooterTemplate>
    ...
    <div class="card">
        <div class="card-title"><span class="ico" style="background:var(--green-lt);color:var(--green)"><i class="fa fa-calendar-check"></i></span>Subject-wise Attendance Detail</div>
        <div class="tbl-wrap">
            <table class="tbl">
                <thead><tr><th>Subject</th><th>Code</th><th>Students</th><th>Classes</th><th>Present</th><th>Avg %</th></tr></thead>
                <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex;align-items:center;gap:8px">
                                        ...
                                        <span style="font-weight:700;font-size:12px;color:<%# AttColor(Eval("AvgAttendancePct")) %>"><%# string.Format("{0:F1}%",Eval("AvgAttendancePct")) %></span>
                                    </div>
                                </td>
                            </tr>
                        </ItemTemplate>
...
<div class="tab-pane" id="tComms">
    <div class="g2">
        <div class="card">
            <div class="card-title"><span class="ico" style="background:var(--blue-lt);color:var(--blue)"><i class="fa fa-stream"></i></span>Activity Log</div>
            <div class="timeline">
        ...
        <div class="card">
            <div class="card-title"><span class="ico" style="background:var(--amber-lt);color:var(--amber)"><i class="fa fa-bell"></i></span>Notifications</div>
            <div class="timeline">
                <asp:Repeater ID="rptNotifs" runat="server">
                    <ItemTemplate>
                        <div class="tl-item">
                            <div class="tl-dot" style="background:<%# Convert.ToBoolean(Eval("IsRead")) ? "var(--ink4)" : "var(--amber)" %>"><i class="fa fa-bell"></i></div>
                            <div class="tl-body">
    ...
    <div class="card">
        <div class="card-title"><span class="ico" style="background:var(--green-lt);color:var(--green)"><i class="fa fa-question-circle"></i></span>Help Requests &amp; Replies</div>
            ...
            <ItemTemplate>
                <div class="help-card">
                    ...
                    <%# Convert.ToBoolean(Eval("HasReply"))
                        ? "<div class='help-reply'><div class='hr-from'><i class='fa fa-user-shield me-1'></i>" + H(Eval("RepliedBy")) + " — " + FmtDateTime(Eval("RepliedOn")) + "</div><div class='hr-text'>" + H(Eval("Reply")) + "</div></div>"
    ...
    <div class="card">
        ...
        <asp:Repeater ID="rptComments" runat="server">
            <ItemTemplate>
                <div class="cmt-card">
                    <div class="cmt-head">
                        <div class="cmt-av" style="background:var(--blue)"><%# H(Eval("CommenterName")).Length>0 ? H(Eval("CommenterName")).Substring(0,1).ToUpper() : "?" %></div>
                        <div>
                            <div class="cmt-name"><%# H(Eval("CommenterName")) %></div>
                            <span class="tag tag-blue" style="font-size:9px"><%# Eval("CommenterRole") %></span>
                        </div>
...
<div class="tab-pane" id="tAnalytics">
    <div class="chart-box" style="margin-bottom:18px">
        <div class="card-title"><i class="fa fa-chart-line" style="color:var(--blue)"></i>&nbsp;Monthly Video Uploads &amp; Student Views Trend</div>
...
document.addEventListener('DOMContentLoaded',function(){
    var map={kSubjects:'<%= hfKpiSubjects.ClientID %>',kVideos:'<%= hfKpiVideos.ClientID %>',kViews:'<%= hfKpiViews.ClientID %>',kAssign:'<%= hfKpiAssignments.ClientID %>',kStudents:'<%= hfKpiStudents.ClientID %>',kAI:'<%= hfKpiAI.ClientID %>'};
    Object.keys(map).forEach(function(k){var el=document.getElementById(k);if(el)countUp(el,parseFloat(hf(map[k]))||0);});
    ...
    var cmt=document.getElementById('cmtBadge');if(cmt)cmt.innerText=hf('<%= hfCommentTotal.ClientID %>')+' comments';
    var ai=document.getElementById('aiBadge');if(ai)ai.innerText=hf('<%= hfAITotal.ClientID %>')+' interactions';
    var asgTot=document.getElementById('asgTotalLbl');if(asgTot)asgTot.innerText=hf('<%= hfAsgTotal.ClientID %>');
    var asgSub=document.getElementById('asgSubsLbl');if(asgSub){var arr=jp('<%= hfAsgSubs.ClientID %>');asgSub.innerText=arr.reduce(function(a,b){return a+b;},0);}

    var pairs=[['<%= lblEmpId.ClientID %>','<%= lblEmpId2.ClientID %>'],['<%= lblDesig.ClientID %>','<%= lblDesig2.ClientID %>'],['<%= lblStream.ClientID %>','<%= lblStream2.ClientID %>'],['<%= lblSession.ClientID %>','<%= lblSession2.ClientID %>'],['<%= lblLastLogin.ClientID %>','<%= lblLastLogin2.ClientID %>']];
    pairs.forEach(function(p){var src=document.getElementById(p[0]);var dst=document.getElementById(p[1]);if(src&&dst)dst.innerHTML=src.innerHTML;});

    var obs=new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting){var f=e.target,w=f.style.width;f.style.width='0';setTimeout(function(){f.style.width=w;},50);obs.unobserve(f);}});},{threshold:.1});
    document.querySelectorAll('.pb-fill,.sec-pbar-fill').forEach(function(el){obs.observe(el);});
...
function countUp(el,target){
    ...
    (function step(now){var p=Math.min((now-start)/dur,1);var e=1-Math.pow(1-p,3);el.textContent=Math.round(target*e).toLocaleString();if(p<1)requestAnimationFrame(step);})(performance.now());
}
...
/* ══ RADAR — uses REAL % values from separate hidden fields ══════════════ */
function buildRadar(){
    var ctx=document.getElementById('radarChart');
    if(!ctx||ctx._c)return;
    /* Each value is a genuine 0-100 percentage:
       Attendance%  = avg attendance across subjects
       Sub%         = assignment submission rate
       VideoComp%   = syllabus completion (videos watched ≥80%)
       Avg Rating   = (AvgRating / 5) * 100 — converted to 0-100 scale
       Students     = scaled as min(students*2, 100) vs institute capacity */
    var att  = parseFloat(hf('<%= hfRadarAttPct.ClientID %>'))  || 0;
    var sub  = parseFloat(hf('<%= hfRadarAsgPct.ClientID %>'))  || 0;
    var vid  = parseFloat(hf('<%= hfRadarVidPct.ClientID %>'))  || 0;
    var rat  = parseFloat(hf('<%= hfRadarRatPct.ClientID %>'))  || 0;  // already ×20
    var stud = parseFloat(hf('<%= hfRadarSubPct.ClientID %>'))  || 0;  // students scaled

    ctx._c=new Chart(ctx,{
        type:'radar',
        data:{
            labels:['Attendance %','Assignment\nSubmission %','Syllabus\nCompletion %','Avg Rating\n(×20 → 0-100)','Students\nReach'],
            datasets:[{
                label:'Performance Score',
                data:[att,sub,vid,rat,stud],
        ...
        options:{
            ...
            scales:{r:{
                min:0,max:100,
                ticks:{stepSize:20,backdropColor:'transparent',font:{size:10},callback:function(v){return v+'%';}},
...
function buildSyllabus(){
    var ctx=document.getElementById('syllabusChart');if(!ctx||ctx._c)return;
    ...
    ctx._c=new Chart(ctx,{type:'bar',data:{labels:L,datasets:[{label:'Completion %',data:D,backgroundColor:'rgba(79,70,229,.75)',borderRadius:7,borderSkipped:false}]},options:bOpts('%','%')});
}
function buildEnroll(){
    var ctx=document.getElementById('enrollChart');if(!ctx||ctx._c)return;
    ...
    ctx._c=new Chart(ctx,{type:'bar',data:{labels:L,datasets:[{label:'Students',data:D,backgroundColor:'rgba(5,150,105,.75)',borderRadius:7,borderSkipped:false}]},options:bOpts('','Students')});
}
function buildRatingChart(){
    var ctx=document.getElementById('ratingChart');if(!ctx||ctx._c)return;
    ...
    ctx._c=new Chart(ctx,{type:'bar',data:{labels:L,datasets:[{label:'Avg Rating',data:D,backgroundColor:'rgba(245,158,11,.8)',borderRadius:6,borderSkipped:false,indexAxis:'y'}]},
        options:{indexAxis:'y',responsive:true,animation:{duration:900},scales:{x:{min:0,max:5,ticks:{font:{size:10},callback:function(v){return v+'★';}},grid:{color:'rgba(0,0,0,.04)'}},y:{grid:{display:false},ticks:{font:{size:10}}}},plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return c.raw+'★';}}}}}});
}
function buildViewsChart(){
    var ctx=document.getElementById('viewsChart');if(!ctx||ctx._c)return;
    var L=jp('<%= hfRatingLabels.ClientID %>'),D=jp('<%= hfRatingViews.ClientID %>');
    if(!L.length){ctx.parentElement.innerHTML=emptyH('No data.');return;}
    ctx._c=new Chart(ctx,{type:'bar',data:{labels:L,datasets:[{label:'Student Views',data:D,backgroundColor:'rgba(8,145,178,.75)',borderRadius:6,borderSkipped:false,indexAxis:'y'}]},
        options:{indexAxis:'y',responsive:true,animation:{duration:900},scales:{x:{ticks:{font:{size:10}},grid:{color:'rgba(0,0,0,.04)'}},y:{grid:{display:false},ticks:{font:{size:10}}}},plugins:{legend:{display:false}}}});
}

/* ══ ASSIGNMENT CHART — legend shown above canvas in HTML ════════════════ */
function buildAsg(){
    var ctx=document.getElementById('asgChart');if(!ctx||ctx._c)return;
    ...
    ctx._c=new Chart(ctx,{type:'bar',
        data:{labels:L,datasets:[
            ...
            {label:'Total Students',data:T,backgroundColor:'rgba(148,163,184,.4)',borderRadius:5,borderSkipped:false}
        ]},
        options:{responsive:true,animation:{duration:900},
            scales:{y:{beginAtZero:true,grid:{color:'rgba(0,0,0,.04)'},ticks:{font:{size:10}}},x:{grid:{display:false},ticks:{font:{size:10,weight:'600'}}}},
            plugins:{legend:{position:'bottom',labels:{font:{size:11},padding:12}},
                tooltip:{callbacks:{label:function(c){return c.dataset.label+': '+c.raw;}}}}}});
}
function buildAtt(){
    var ctx=document.getElementById('attChart');if(!ctx||ctx._c)return;
    ...
    ctx._c=new Chart(ctx,{type:'bar',data:{labels:L,datasets:[{label:'Attendance %',data:D,backgroundColor:colors,borderRadius:7,borderSkipped:false}]},options:bOpts('%','Att %')});
}
function buildTrend(){
    var ctx=document.getElementById('trendChart');if(!ctx||ctx._c)return;
    var L=jp('<%= hfTrendLabels.ClientID %>'),C=jp('<%= hfTrendCounts.ClientID %>'),V=jp('<%= hfTrendViews.ClientID %>');
        ...
        ctx._c = new Chart(ctx, {
            type: 'bar', data: {
                labels: L, datasets: [
                    { label: 'Videos Uploaded', data: C, backgroundColor: 'rgba(79,70,229,.75)', borderRadius: 6, borderSkipped: false, yAxisID: 'y' },
                    { label: 'Student Views', data: V, type: 'line', borderColor: '#f59e0b', backgroundColor: 'rgba(245,158,11,.1)', borderWidth: 2.5, pointRadius: 4, tension: .4, fill: true, yAxisID: 'y1' }
                ]
            }, options: {
    ...
    function bOpts(sfx, yLbl) {
        return {
            ...
            scales: { y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,.04)' }, ticks: { font: { size: 10 }, callback: function (v) { return v + sfx; } }, title: { display: true, text: yLbl, font: { size: 10 } } }, x: { grid: { display: false }, ticks: { font: { size: 10, weight: '600' } } } },
            plugins: { legend: { display: false }, tooltip: { callbacks: { label: function (c) { return c.raw + sfx; } } } }
        };
    }
    function emptyH(msg) { return '<div class="empty"><i class="fa fa-chart-bar"></i><p>' + msg + '</p></div>'; }
    function toggleAcc(hd) { hd.classList.toggle('open'); var b = hd.nextElementSibling; if (b && b.classList.contains('acc-body')) b.classList.toggle('open'); }
</script>
</asp:Content>
