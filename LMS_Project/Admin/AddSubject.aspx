...
<div class="subject-page-header mb-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap header-actions">
            ...
            <!-- Add Button — hidden for SuperAdmin -->
        ...
        <asp:GridView ID="gvSubjects" runat="server"
            ...
            <EmptyDataTemplate>
                <div class="text-center py-5">
                    ...
                    <p class="text-muted small">No subjects added yet. Click the <strong>Add Subject</strong> button to add new subjects.</p>
                </div>
            </EmptyDataTemplate>
            ...
            <div class="modal-body p-4">
                <div class="row g-3">
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <div class="invalid-feedback" id="codeError">Only letters and numbers allowed (no special characters).</div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <div class="form-text text-muted" style="font-size:11px">Numeric value only (e.g. 45 mins, 2 hrs)</div>
                    </div>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <div class="form-check form-switch mt-2">
                            ...
                            <label class="form-check-label" for="<%= chkActive.ClientID %>">
                                Active
                            </label>
                        </div>
                    </div>
...
<script>
    ...
    function showToast(msg, type) {
        ...
        var bsToast = bootstrap.Toast.getOrCreateInstance(t, { delay: 4000 });
        bsToast.show();
    }
    ...
    function filterTable(val) {
        ...
        var rows = document.querySelectorAll('#<%= gvSubjects.ClientID %> tbody tr');
        var found = 0;
        rows.forEach(function (r) {
            var show = r.innerText.toLowerCase().includes(val);
    ...
    function openAddModal() {
        if (isSuperAdmin) {
            showToast('SuperAdmin has view-only access. Cannot add subjects.', 'warning');
            return;
        }
        document.getElementById('<%= hfSubjectId.ClientID %>').value = '';
    document.getElementById('<%= txtSubjectCode.ClientID %>').value = '';
    document.getElementById('<%= txtSubjectName.ClientID %>').value = '';
    document.getElementById('<%= txtDurationValue.ClientID %>').value = '';
    document.getElementById('<%= txtDescription.ClientID %>').value = '';
...
// Called from code-behind for Edit
function openModal() {
    clearAllErrors();
...
function confirmDelete(btn) {
    if (isSuperAdmin) {
        showToast('SuperAdmin has view-only access. Cannot delete subjects.', 'warning');
        return false;
    }
    return confirm('Are you sure you want to delete this subject?\n\nNote: If this subject is used elsewhere, it cannot be deleted.');
}
...
// Subject Code: alphanumeric only
...
// Duration: positive integer only
...
function validateForm() {
    if (isSuperAdmin) {
        showToast('SuperAdmin has view-only access. Cannot save subjects.', 'warning');
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --------- Server-side Toast Bridge ---------
    // Called by code-behind via ScriptManager to show server toasts
