...
namespace LearningManagementSystem.Teacher
{
    public partial class Notifications : BasePage
    {
        ...
        private void HandleAjax(string action)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("[TeacherNotif] " + ex);
        ...
        // Types scoped to this teacher's received notifications only
        ...
        {
            int id = GetI("id");
            if (id > 0) _bl.MarkRead(id, uid);
            return new { ok = true };
        }

        private object DoMarkAll(int inst, int sess, int uid)
        {
            _bl.MarkAllRead(inst, sess, uid);
            return new { ok = true };
        }
        ...
        {
            int id = GetI("id");
            if (id > 0) _bl.DeleteOne(id, uid);
            return new { ok = true };
        }

        private object DoDeleteAll(int inst, int sess, int uid)
        {
            _bl.DeleteAll(inst, sess, uid);
            return new { ok = true };
        }
