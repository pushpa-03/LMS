...
namespace LMS_Project.Teacher
{
    ...
    {
        // ── File-size limits ──────────────────────────────────────────────────
        //private const long VIDEO_MAX_BYTES = 500L * 1024 * 1024; // 500 MB
        //private const long MATERIAL_MAX_BYTES = 50L * 1024 * 1024; //  50 MB
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    {
                        Response.Redirect("Subjects_T_Side.aspx");
                        return;
                    }
        ...
        protected void rptChapters_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            catch { /* silently ignore per-row binding errors */ }
        }
        ...
        protected void rptVideos_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            catch (Exception ex) { ShowMsg("Error deleting video: " + ex.Message, "danger"); }
        }
        ...
        protected void rptMaterials_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            catch (Exception ex) { ShowMsg("Error deleting material: " + ex.Message, "danger"); }
        }
        ...
        protected void btnSaveChapter_Click(object sender, EventArgs e)
        {
            ...
            { ShowMsg("Chapter name is required.", "warning"); return; }
            try
            {
                int subjectId = Convert.ToInt32(hfSubjectId.Value);
                _bl.SaveChapter(
                    hfChapterId.Value, SessionId, subjectId.ToString(),
        ...
        protected void btnUploadSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (contentType == "Video")
                {
                    ...
                    {
                        ShowMsg($"Video file is too large ({FormatBytes(fileSize)}). Maximum allowed size is 500 MB.", "warning");
                        return;
                    }
                ...
                else
                {
                    ...
                    {
                        ShowMsg($"Material file is too large ({FormatBytes(fileSize)}). Maximum allowed size is 50 MB.", "warning");
                        return;
                    }
                    ValidateMaterialFile(ext);
                }
        ...
        //  Called from JS:  fetch('SubjectDetails_T_Side.aspx/SearchTeachers', ...)
        // ═════════════════════════════════════════════════════════════════════
        ...
        // ═════════════════════════════════════════════════════════════════════
        //  MARKUP HELPERS  (called from .aspx DataBinding expressions)
        // ═════════════════════════════════════════════════════════════════════
        ...
        public string GetMaterialUrl(object filePath)
        {
