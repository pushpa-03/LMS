<%@ Page Title="Subject Details" Language="C#" MasterPageFile="~/Teacher/TeacherMaster.master"
    AutoEventWireup="true" CodeBehind="SubjectDetails_T_Side.aspx.cs"
    Inherits="LMS_Project.Teacher.SubjectDetails" %>
...
:root {
    ...
    --border:#e4e7f0; --accent:#c62828; --accent-2:#7c3aed;
...
.info-card-header { background:linear-gradient(135deg,#c62828,#7c3aed); padding:24px 28px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; }
...
.video-row:hover { border-color:var(--accent); background:#fff5f5; }
.video-row-left { display:flex; align-items:center; gap:10px; }
.vid-thumb { width:48px; height:34px; background:#7c1d1d; border-radius:6px; display:flex; align-items:center; justify-content:center; color:#fff; font-size:.9rem; flex-shrink:0; }
...
.btn-sm-icon.watch { background:#fff0f0; color:var(--accent); }
...
.badge-marks { background:#fff0f0; color:var(--accent); border-radius:6px; padding:2px 8px; font-weight:700; }
...
.form-control-custom:focus { border-color:var(--accent); outline:none; box-shadow:0 0 0 3px rgba(198,40,40,.12); }
...
.form-select-custom:focus { border-color:var(--accent); outline:none; box-shadow:0 0 0 3px rgba(198,40,40,.12); }
...
.selected-teacher { display:flex; align-items:center; gap:8px; background:#fff0f0; border:1px solid #fca5a5; border-radius:8px; padding:8px 12px; margin-top:8px; font-size:.82rem; }
...
<!-- TOP NAV -->
<div class="top-nav">
    <a href="Subjects_T_Side.aspx" class="btn-back"><i class="fa fa-arrow-left"></i> Back</a>
    <h2><i class="fa fa-book-open me-2"></i>
...
<div class="layout">
    ...
    <div>
        ...
        <div class="section-hdr">
            ...
            <span id="chapterCountBadge" style="background:#fff0f0;color:var(--accent);border-radius:8px;padding:4px 12px;font-size:12px;font-weight:700;"></span>
        </div>
                        ...
                        <div class="chapter-body" style="display:none">
                            ...
                            <div class="ch-tab-pane active" id="vid<%# Eval("ChapterId") %>">
                                <asp:Repeater ID="rptVideos" runat="server" OnItemCommand="rptVideos_ItemCommand">
                                    <ItemTemplate>
                                        <div class="video-row">
                                            ...
                                            <div class="video-row-right">
                                                <a href='TeacherVideoPlayer.aspx?VideoId=<%# Eval("VideoId") %>' class="btn-sm-icon watch" title="Watch"><i class="fa fa-play"></i></a>
                                    ...
                                    <FooterTemplate>
                                        <asp:PlaceHolder ID="phNoVid" runat="server" Visible='<%# Container.ItemIndex < 0 %>'>
                            ...
                            <div class="ch-tab-pane" id="mat<%# Eval("ChapterId") %>">
                                <asp:Repeater ID="rptMaterials" runat="server" OnItemCommand="rptMaterials_ItemCommand">
                                    <ItemTemplate>
                                        <div class="mat-row">
                                            ...
                                            <div style="display:flex;gap:6px">
                                                <a href='<%# GetMaterialUrl(Eval("FilePath")) %>' target="_blank" class="btn-sm-icon watch" title="View"><i class="fa fa-eye"></i></a>
                                    ...
                                    <FooterTemplate>
                                        <asp:PlaceHolder ID="phNoMat" runat="server" Visible='<%# Container.ItemIndex < 0 %>'>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <asp:DropDownList ID="ddlContentType" runat="server" CssClass="form-select-custom" onchange="toggleContentFields()">
                            ...
                            <asp:ListItem Text="📄 Material (PDF/Doc/Image…)" Value="Material" />
                        </asp:DropDownList>
                    </div>
                    <div class="col-12">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="teacher-search-wrap">
                                <input type="text" id="txtTeacherSearch" class="form-control-custom"
                                       placeholder="Search teacher by name…"
                                       oninput="searchTeachers(this.value)" autocomplete="off" />
                        ...
                        <div class="col-md-6">
                            ...
                            <asp:TextBox ID="txtVideoDesc" runat="server" CssClass="form-control-custom"
                                         TextMode="MultiLine" Rows="3" placeholder="Brief description or instructor bio…" />
                        </div>
                    </div>
                ...
                <div class="mt-3">
                    ...
                    <asp:FileUpload ID="fuContent" runat="server" CssClass="form-control-custom" style="padding:6px" onchange="validateFileSize(this)" />

                    <!-- File size limit info — shown dynamically -->
...
const MATERIAL_MAX_BYTES =  50 * 1024 * 1024; //  50 MB
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    toggleContentFields(); // init badge visibility
});
...
// ── Content type toggle (also updates file-size badge) ──
...
function validateFileSize(input) {
    ...
    const limitMB = isVideo ? '500 MB' : '50 MB';
    if (file.size > limit) {
        err.textContent = `File is too large (${formatBytes(file.size)}). Maximum allowed is ${limitMB}.`;
...
function doTeacherSearch(q) {
    ...
    fetch('SubjectDetails_T_Side.aspx/SearchTeachers', {
        method : 'POST',
